...
def getQuery(queryIndex):
    return {
        ...
        10: "SELECT id, activities AS activity, examination_id, created_at, updated_at FROM psychological_plans WHERE examination_id IN (SELECT id FROM examinations WHERE doctor_id IS NOT NULL AND doctor_id IN (SELECT id FROM doctors WHERE user_id IS NOT NULL)) ORDER BY created_at ASC;",
        ...
        15: 'SELECT id, feel_culpability, feel_infamous, feel_pointless, feel_sad, feel_unhappy, feel_useless, no_pleasure, really_am AS eally_am, CASE WHEN result = "Normal" THEN 1 WHEN result = "Soportable" THEN 2 WHEN result = "Depresion" THEN 3 WHEN result = "Depresion mayor" THEN 4 ELSE NULL END AS result, spoil, time_spent, session_id, created_at, updated_at FROM depression_tests WHERE session_id IS NOT NULL AND session_id IN (SELECT id FROM sessions WHERE person_id IN (SELECT id FROM people WHERE user_id IS NOT NULL)) ORDER BY created_at ASC;',
        ...
        18: 'SELECT id, fevone, fevoneratio, pkf, recomendation, CASE WHEN result = "Nivel normal" THEN 1 WHEN result = "Resultado moderado" THEN 2  WHEN result = "Resultado Severo" THEN 3 ELSE NULL END AS result, CAST(total_lm AS UNSIGNED) AS total_lm, session_id, created_at, updated_at FROM spirometers WHERE session_id IS NOT NULL AND session_id IN (SELECT id FROM sessions WHERE person_id IN (SELECT id FROM people WHERE user_id IS NOT NULL)) ORDER BY created_at ASC;',
...
def copyToDB(table, cols):
    ...
    os.putenv("PGPASSWORD", os.getenv("PG_PASS"))
    ...
    for index, table in tables.items():
        try:
            ...
            if index == 20:
                ...
                df["sleep_driving_frecuence"] = df["sleep_driving_frecuence"].astype(
                    "int"
                )
