...
def getQuery(queryIndex)
  {
    ...
    1 => 'SELECT users.id, users.approved_terms_for_examinations, users.auth_token, people.birth_date as birthday, users.current_sign_in_at, users.current_sign_in_ip, users.email, users.encrypted_password, people.last_name, users.last_sign_in_at, users.last_sign_in_ip, people.name, users.passwd_login, users.people_filters, people.phone_number AS phone, people.phone_area_code, users.remember_created_at, users.reset_password_token, users.reset_password_sent_at, CASE WHEN users.role = "admin" THEN 1 WHEN users.role = "general" THEN 2 WHEN users.role = "doctor" THEN 3 WHEN users.role = "employee" THEN 4 WHEN users.role = "evaluator" THEN 5 ELSE NULL END AS role, people.is_male AS sex, users.sign_in_count, users.stripe_id, users.stripe_subscription_id, CASE WHEN people.neighbourhood_id > 0 THEN 3166 ELSE 3166 END AS country_id, users.created_at, users.updated_at FROM users, people WHERE people.user_id = users.id ORDER BY users.created_at ASC;',
    2 => 'SELECT id, has_children AS children, has_credit_card AS credit_card, private_insurance AS insurance, profession, race, user_id, created_at, updated_at FROM people WHERE user_id IS NOT NULL ORDER BY created_at ASC;',
    ...
    5 => 'SELECT id, person_id, created_at, updated_at FROM sessions WHERE person_id IN (SELECT id FROM people WHERE user_id IS NOT NULL) ORDER BY created_at ASC;',
