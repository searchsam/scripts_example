...
def getQuery(queryIndex)
  {
    ...
    1 => 'SELECT users.id, users.auth_token, people.birth_date as birthday, users.current_sign_in_at, users.current_sign_in_ip, "", users.email, users.encrypted_password, users.last_sign_in_at, users.last_sign_in_ip, users.passwd_login, people.phone_number AS phone, users.remember_created_at,  users.reset_password_sent_at, users.reset_password_token, CASE WHEN users.role = "admin" THEN 1 WHEN users.role = "general" THEN 2 WHEN users.role = "doctor" THEN 3 WHEN users.role = "employee" THEN 4 WHEN users.role = "evaluator" THEN 5 ELSE NULL END AS role, users.sign_in_count, CASE WHEN people.neighbourhood_id > 0 THEN 154 ELSE 154 END AS country_id, users.created_at, users.updated_at FROM users, people WHERE people.user_id = users.id ORDER BY users.created_at ASC;',
    ...
    13 => 'SELECT id, CAST(mind AS UNSIGNED) AS mind, risk_category AS risk, session_id, created_at, updated_at FROM emotional_states ORDER BY created_at ASC;',
    ...
    24 => 'SELECT id, CASE WHEN ayuna = 1 THEN TRUE WHEN ayuna = 0 THEN FALSE ELSE NULL END AS fast, CAST(level AS UNSIGNED) AS level, recomendation, CASE WHEN result = "Nivel óptimo" THEN TRUE WHEN result = "Nivel no óptimo" THEN FALSE WHEN result = "<h4 style=\"color: green;\">Nivel óptimo</h4>" THEN TRUE WHEN result = "<h4 style=\"color: red;\">Nivel no óptimo</h4>" THEN FALSE ELSE NULL END AS result, time_spent, session_id, created_at, updated_at FROM glucose_levels WHERE session_id IS NOT NULL AND session_id IN (SELECT id FROM sessions WHERE person_id IN (SELECT id FROM people WHERE user_id IS NOT NULL)) ORDER BY created_at ASC;',
    25 => 'SELECT id, gmdl, CAST(hct AS UNSIGNED) AS hct, recomendation, CASE WHEN result = "Nivel Bajo" THEN 1 WHEN result = "Normal" THEN 2 WHEN result = "Elevado" THEN 3 ELSE NULL END AS result, time_spent, session_id, created_at, updated_at FROM hemoglobins WHERE session_id IS NOT NULL AND session_id IN (SELECT id FROM sessions WHERE person_id IN (SELECT id FROM people WHERE user_id IS NOT NULL)) ORDER BY created_at ASC;',
    26 => 'SELECT id, fevone, fevoneratio, pkf, recomendation, CASE WHEN result = "Resultado moderado" THEN 1 WHEN result = "Nivel normal" THEN 2 WHEN result = "Resultado Severo" THEN 3 WHEN result = "<h3 style=\"color: green;\">Nivel normal</h3>" THEN 2 WHEN result = "<h3 style=\"color: red\">Resultado moderado</h3>" THEN 1 ELSE NULL END AS result, CAST(total_lm AS UNSIGNED) AS total_lm, session_id, created_at, updated_at FROM spirometers WHERE session_id IS NOT NULL AND session_id IN (SELECT id FROM sessions WHERE person_id IN (SELECT id FROM people WHERE user_id IS NOT NULL)) ORDER BY created_at ASC;',
    27 => 'SELECT id, CAST(level AS UNSIGNED) level, recomendation, CASE WHEN result = "Elevado" THEN TRUE WHEN result = "<h3 id=\"exam_bad_result\">Elevado</h4>" THEN TRUE WHEN result = "Normal" THEN FALSE WHEN result = "<h3 style=\"color: green;\">Normal</h3>" THEN FALSE ELSE NULL END AS result, time_spent, session_id, created_at, updated_at FROM triglyceride_levels WHERE session_id IS NOT NULL AND session_id IN (SELECT id FROM sessions WHERE person_id IN (SELECT id FROM people WHERE user_id IS NOT NULL)) ORDER BY created_at ASC;',
