...
def getQuery(queryIndex)
  {
    ...
    7 => 'SELECT id, finished_at, finished, specialty, taken, twilio_channel_name, twilio_channel_sid, CASE WHEN service_type = "free" THEN 1 WHEN service_type = "paid" THEN 2 ELSE NULL END AS type, doctor_id, person_id, created_at, updated_at FROM examinations ORDER BY created_at ASC;',
    8 => 'SELECT id, DATE(appointment_date) AS date, TIME(appointment_date) AS hour, reason, specialty, CASE WHEN status = "scheduled" THEN 1 WHEN status = "pending" THEN 2 WHEN status = "finished" THEN 3 ELSE NULL END AS status, taken, CASE WHEN ap_type = "plan" THEN 1 ELSE NULL END AS type, person_id,doctor_id, created_at, updated_at FROM appointments ORDER BY created_at ASC;',
